var dragndrop = (function(){
	...
	function createDragImg(){
		...
		draggableImage.setAttribute('class', letters[count] + " " + colors[count]);
	...
	for (i = 0; i < letters.length; i++){
		...
		dzImage.setAttribute('class', letters[i]);
	...
	function setLetterValue(){
		...
		window.letter = this.className[0];
		window.color = this.className[2] + this.className[3] + this.className[4];
	}
    ...
    let dzSrc = dz.className;
		...
		if( window.letter === dzSrc ){
			...
			playSound(window.letter.toUpperCase());
			test++;
			letterTest();
			createDragImg();
		}
